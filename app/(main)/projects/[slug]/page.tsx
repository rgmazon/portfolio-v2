...
export default function ProjectPage() {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowRight") setLightboxIndex((i) => (i + 1) % (project?.media.length ?? 1));
      if (e.key === "ArrowLeft") setLightboxIndex((i) => (i - 1 + (project?.media.length ?? 1)) % (project?.media.length ?? 1));
    };
  ...
  if (project === undefined) {
    return (
      <div style={{ background: "var(--bg)", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <span style={{ fontFamily: "var(--font-body), sans-serif", fontSize: 14, color: "var(--muted)", letterSpacing: "0.08em", textTransform: "uppercase" }}>
          Loading…
        </span>
      </div>
    );
